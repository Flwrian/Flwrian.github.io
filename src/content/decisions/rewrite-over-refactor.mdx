...
date: 2024-03-10
context: "Aspira v1 'worked' - it played legal chess moves and beat random players. But it had fundamental architectural issues: move generation was inefficient, board representation caused unnecessary allocations, and the search algorithm had subtle bugs. Should I refactor incrementally or rewrite from scratch?"
decision: "Complete rewrite of Aspira, throwing away months of work to build on a better foundation."
alternatives:
...
tags:
  ...
  - "Technical Debt"
  - "Software Design"
outcome: "The rewrite took significant time but resulted in dramatically better code. v2 is faster (20M+ nodes/sec vs ~8M), more correct (passes extensive perft suites), and more maintainable (easier to add features). Performance gains came from simpler, cleaner code rather than clever tricks. The solid foundation makes future improvements straightforward instead of fighting the architecture."
relatedProjects:
  - "aspira-chess-engine"
relatedDecisions:
...
Aspira v1 worked. I could play it against other engines. It made legal moves, evaluated positions, managed time control.

But deep down, I knew the architecture was wrong.

### The Problems

**Move Generation**: Generated pseudo-legal moves then filtered for legality. Inefficient and error-prone.

**Board Representation**: Mixed bitboards and piece lists inconsistently. Caused unnecessary allocations and synchronization bugs.

**Make/Unmake**: Subtle bugs in position restoration after trying moves. Would corrupt state several plies deep.

**Search**: Alpha-beta implementation had edge cases where it didn't prune correctly. Hard to extend.
...
- Fix search → back to move generation issues

It was like pulling on a thread and watching the whole sweater unravel.

## The Decision Moment

After three weeks of fighting the architecture, I asked myself:
...
Throwing away months of work hurts. You have functioning code. It does things. People can use it.
...
v2 prioritized correctness, which paradoxically led to better performance (fewer bugs, simpler code that JIT can optimize).
...
- **v1**: ~8M nodes/sec with bugs
- **v2**: 20-22M nodes/sec, correct
